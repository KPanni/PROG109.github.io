...
function userGuessed() {
	...
	if (userGuessed.length == 0 || ! guessInRange(userGuessed)) {
		...
		writeMessage('statusArea', '<p>Please enter a number between1-20 and press the Guess button.</p>');
	} else if (userGuessed.indexOf('.') != -1) {
		writeMessage('statusArea', '<p>Please enter a whole number between 1-20 and press the Guess button.</p>');
	} else {
		numberOfGuesses++;
